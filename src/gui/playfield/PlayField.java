...
public class PlayField extends JPanel {
	...
	public void setBackground(String path, double scaleFactor) throws IOException {
		this.backgroundGraphic = new ImageGraphic(path, scaleFactor);
	}

	public void setPolygons(List<List<Coord>> coordSets) {
		this.polygonGraphics = coordSets.stream().map(PolygonGraphic::new).toList();
	}
	...
	public void paint(Graphics g) {
		...
		if (backgroundGraphic != null) {
			backgroundGraphic.draw(g2);
		}

		// draw map (if exists and drawing requested)
		if (mapGraphic != null && showMapGraphic) {
			mapGraphic.draw(g2);
		}

		if (polygonGraphics != null) {
			for(PolygonGraphic p : polygonGraphics) {
				p.draw(g2);
			}
		}
